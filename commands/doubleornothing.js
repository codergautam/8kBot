...
module.exports = {
    ...
    execute(message, args) {
        ...
        .then((user) => {  
            api.checkCool(message.author.id, "doubleornothing")
            .then((cooldown) => {
                if(cooldown.cooldown) {
                    const embed = new Discord.MessageEmbed()
                    .setColor('#0099ff')
                    .setTitle("Cooldown")
                    .setDescription("You just played Double Or Nothing\nYou can play again in `"+api.convertMS(cooldown.msleft)+"`")
        ...
        .catch((err) => {
            message.channel.send("an error occured try this command again\n")
        })
    })
