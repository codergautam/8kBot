    ...
    work(message, callback, user) {
        ...
        if (!user.inv.hasOwnProperty("laptop") || !user.inv.hasOwnProperty("headset")) {
            ...
            if(user.youtube.subs > 1000000) {
                var myArray = [3,3, 9,9,3 ,10,1,2,6,6 ];   
            }
           ...
           if(moneyEarned * moneyMultiplier > 0) embed.setDescription(embed.description += `\nYou earned \`${api.numberWithCommas(moneyEarned * moneyMultiplier)}\` coins!`)
